        ...
        while IFS= read -r numbered_line; do
            ...
            if [[ ! -z "$line" ]]; then
                ...
                if [[ "$line" =~ ^[[:space:]]*# ]]; then
                    ...
                    echo "--✏️ Редактировать | shell=\"$0\" param1=\"_edit_dialog\" param2=\"$line_num\" param3=\"$escaped_line\" refresh=true"
...
show_edit_dialog() {
    ...
    # Показываем диалог редактирования через osascript
    osascript <<EOF
    tell application "System Events"
        set theDialog to display dialog "Редактирование cron задания:\n\nТекущее задание:\n$current_job\n\nВведите новое задание:" default answer "$current_job" with title "Редактирование Cron"
...
# Функция для показа диалога создания
show_create_dialog() {
    osascript <<EOF
    tell application "System Events"
        set theDialog to display dialog "Создание нового cron задания:\n\nФормат: минута час день_месяца месяц день_недели команда\n\nПримеры:\n* * * * *    /path/to/script.sh\n0 * * * *    Каждый час\n0 9 * * *    Каждый день в 9:00\n0 9 * * 1    Каждый понедельник в 9:00" default answer "" with title "Новое Cron задание"
        set theAnswer to text returned of theDialog
    end tell
    return theAnswer
EOF
...
# Функция для открытия Terminal с командой
...
main_menu() {
    ...
    echo "---"
    echo "⚙️ Управление"
    echo "✏️ Новое задание | shell=\"$0\" param1=\"_create_dialog\" refresh=true"
    echo "📋 Просмотреть все | shell=\"$0\" param1=\"_view_crontab\""
    echo "✏️ Редактировать все | shell=\"$0\" param1=\"_edit_crontab\""
...
case "${1}" in
    "process_cron_edit")
        process_cron_edit "$2" "$3" "$4"
        ;;
    "process_cron_create")
        process_cron_create "$2"
        ;;
    "_edit_dialog")
        new_job=$(show_edit_dialog "$2" "$3")
        if [ -n "$new_job" ]; then
            process_cron_edit "$new_job" "$2" "$3"
        fi
        ;;
    "_create_dialog")
        new_job=$(show_create_dialog)
        if [ -n "$new_job" ]; then
            process_cron_create "$new_job"
        fi
        ;;
    "_view_crontab")
        open_terminal_with_command "crontab -l"
        ;;
    "_edit_crontab")
        open_terminal_with_command "crontab -e"
        ;;
    *)
        main_menu
        ;;
