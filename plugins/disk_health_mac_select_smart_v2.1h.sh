...
get_temperature() {
    ...
    # 1. Ищем в атрибутах SMART (поле RAW_VALUE) - исправленная версия
    temp_attr=$(echo "$smart_info" | grep -E "^[[:space:]]*(190|194|197|198|199)" | grep -v "0[[:space:]]*\-" | awk '{if ($10 ~ /^[0-9]+$/) print $10}' | head -1)
    
    if [ -n "$temp_attr" ] && [ "$temp_attr" -gt 0 ] && [ "$temp_attr" -lt 150 ] 2>/dev/null; then
        echo "$temp_attr"
        return 0
    fi
    ...
    if [ -n "$temp_line" ]; then
        ...
        if [ -n "$temp_value" ] && [ "$temp_value" -gt 0 ] && [ "$temp_value" -lt 150 ] 2>/dev/null; then
            echo "$temp_value"
            return 0
        fi
    fi
    ...
    if [ -n "$temp_current" ] && [ "$temp_current" -gt 0 ] && [ "$temp_current" -lt 150 ] 2>/dev/null; then
        echo "$temp_current"
        return 0
    fi
    ...
    if [ -n "$temp_airflow" ] && [ "$temp_airflow" -gt 0 ] && [ "$temp_airflow" -lt 150 ] 2>/dev/null; then
        echo "$temp_airflow"
        return 0
    fi
...
get_attr_value() {
    ...
    # Проверяем, что значение действительно числовое
    if [[ "$value" =~ ^[0-9]+$ ]]; then
        echo "$value"
    else
        echo ""
...
for disk in "${SMART_DISKS[@]}"; do
    ...
    if [ "$disk" = "$SELECTED_DISK" ]; then
        ...
        echo "○ $disk_name | shell=\"$0\" _select_disk $disk refresh=true"
    fi
done
